...
**正解**：
题目的正确作答：单选为一条正确选项文本，多选为 2–3 条互异正解文本，填空为 texts，其它主观题为小问得分点。
...
**GCRDG**：
选择题干扰项流水线：过生成（约 8–12 候选）→ 语义过滤（相对全部正解过近、离材料过远、候选互近则丢）→ 候选验伪 → 按题型取干扰项再组四选项并旋转位置。单选取 3 个干扰（1 正解）；多选取 `4 − n` 个干扰（2–3 个正解）。不足所需个数则无选项待审草稿，禁止占位干扰项。
_Avoid_: 直接编选项, 一次生成 ABCD, 占位选项

**过生成**：
Critic 按题干、全部正解与待考查文本大量写出干扰候选。
...
**质量门控**：
入库前的规则+Critic 检查：答案存在性、选择题正解集合完整且干扰项均非正解、题干泄答、可用性、争议性。不通过则 `needs_review`。
_Avoid_: QA, lint 题目
