...
# 判定规则
- `accepted`：候选与题干同维度，能从待考查文本找到反证或明确的错用条件，且不是任何一条已给出的正解。
- `equivalent_to_answer`：候选是任一条正解的同义改写、近义替换、词序变化或不改变结论的表述，必须拒绝。
- `possibly_correct`：候选也可能由材料支持、条件不足以排除、或会形成额外的合理正解，必须拒绝。
