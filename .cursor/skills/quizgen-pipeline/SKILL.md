...
1. **Generator 只出题干 + 正解。** 选择题干扰项不得在 `generate_stem` / `quiz_generator` 里直接编造。必须走 GCRDG：`overgenerate` → `filter_candidates` → `rank_candidates`（见 `distractor_engine.build_choice_question`）。
2. **长 prompt 只放仓库根目录 `prompts/*.md`。** 用 `prompt_loader.load_prompt(name)` 读取。禁止把长系统提示埋进 Python 字符串。
3. **LLM 只经 `app/services/llm`。** 业务代码调用 `complete_json` / `generator_provider` / `critic_provider`。禁止直连 SDK。Generator（题干）与 Critic（过生成/对抗/质量评判）走不同 provider。
...
解析文档 → 篇章映射（不适切段落跳过）→ 抽关键句 → 蓝图分配微技能/题型 → 生成题干+正解 → GCRDG 组选项 → 对抗修补 → 质量门控 → 入库。

判断题、填空题不走过生成；选择题候选不足 3 个才允许占位并必须 `needs_review=True`。
...
| GCRDG | `distractor_engine.py` + `ranker.py`，prompt：`overgenerate_distractors.md`、`adversarial_review.md` |
